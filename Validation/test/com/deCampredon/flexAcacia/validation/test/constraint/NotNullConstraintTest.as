package com.deCampredon.flexAcacia.validation.test.constraint
{
	import com.deCampredon.flexAcacia.validation.constraints.NotNullConstraint;
	
	import mx.events.ValidationResultEvent;
	
	import org.flexunit.Assert;
	import org.flexunit.asserts.assertEquals;
	import org.flexunit.asserts.assertNotNull;
	import org.flexunit.asserts.assertTrue;
	import org.flexunit.async.Async;
	
	public class NotNullConstraintTest
	{
		
		public var testItem:ConstraintTestItem;
		public var notNullConstraint:NotNullConstraint;
		
		[Before]
		public function setUp():void
		{
			testItem = new ConstraintTestItem();
			
			notNullConstraint = new NotNullConstraint();
			
		}
		
		[Test(async)]
		/**
		 * test than the constraint against a valid email
		 */
		public function validationSuccess():void {
			notNullConstraint.field = "stringProperty";
			testItem.stringProperty = "hello";
			notNullConstraint.addEventListener(ValidationResultEvent.VALID,Async.asyncHandler(this, handle_validationSuccessComplete, 20, null, handle_Timeout));
			notNullConstraint.validate(testItem);
		}	
		
		
		
		[Test(async)]
		public function validationFailWithNull():void {
			notNullConstraint.field = "stringProperty";
			testItem.stringProperty = null;
			notNullConstraint.addEventListener(ValidationResultEvent.INVALID,Async.asyncHandler(this, handle_validationFailComplete, 20, null, handle_Timeout));
			notNullConstraint.validate(testItem);
		}	
		
		
		[Test(async)]
		public function validationFailWithEmptyString():void {
			notNullConstraint.field = "stringProperty";
			testItem.stringProperty = "";
			notNullConstraint.addEventListener(ValidationResultEvent.INVALID,Async.asyncHandler(this, handle_validationFailComplete, 20, null, handle_Timeout));
			notNullConstraint.validate(testItem);
		}	
		
		[Test(async)]
		public function validationFailWithNaN():void {
			notNullConstraint.field = "numberProperty";
			notNullConstraint.addEventListener(ValidationResultEvent.INVALID,Async.asyncHandler(this, handle_validationFailComplete, 20, null, handle_Timeout));
			notNullConstraint.validate(testItem);
		}	
		
		
		/**
		 * Handle validaton success 
		 */
		protected function handle_validationSuccessComplete(event:ValidationResultEvent,passThroughData:Object):void {
			assertTrue(!event.results);
		}
		
		/**
		 * Handle validaton failure
		 */
		protected function handle_validationFailComplete(event:ValidationResultEvent,passThroughData:Object):void {
			assertNotNull(event.results);
			assertEquals(event.results.length,1);
			assertEquals(event.results[0].errorCode,'requiredField');
		}
		
		/**
		 * Handle timeout
		 */
		protected function handle_Timeout(passThroughData:Object):void {
			Assert.fail( "Timeout reached before event");
		}
	}
}